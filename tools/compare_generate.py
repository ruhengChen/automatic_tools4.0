...
def deal_table_add(tablelist, date):
    ...
    for tablename in tablelist:
        ...
        # 0712 修改 job.log("INSERT INTO ETL.JOB_SEQ (JOB_NM,PRE_JOB ,PPN_TSTMP) VALUES ('AP_ODS_%s_YATOPUPDATE','LD_ODS_%s',CURRENT TIMESTAMP);" %(tablestr,tablestr))
...
def get_column_detail(tablename,date):
    syscode, tablenm = tablename.split('.')
    sql = "select trim(field_code),trim(data_tp)||','||trim(length)||','||trim(precsn),case when primary_key_flag='' then 'N' else 'Y' end from DSA.ORGIN_TABLE_DETAIL where trim(src_stm_id) ='{2}' and trim(tab_code) = '{0}' and change_date='{1}'".format(tablenm,date,syscode)
    cursor_dw.execute(sql)
    rows = cursor_dw.fetchall()
    old_column_list = []
    old_column_dict = {}
...
def generate_delta_ddl(tablename,date,tabspace,tabletype):
    ...
    syscode, tablenm = tablename.split('.')
    sql = "select trim(field_code),trim(data_tp),trim(length),trim(precsn),case when primary_key_flag='' then 'N' else 'Y' end, trim(tab_nm),trim(field_nm) from DSA.ORGIN_TABLE_DETAIL where trim(src_stm_id) ='{2}' and trim(tab_code) = '{0}' and change_date='{1}' order by cast(column_id as int)".format(tablenm,date,syscode)
    cursor_dw.execute(sql)
    rows = cursor_dw.fetchall()
...
def deal_column_update(table, newdate, olddate, update_list, is_primary, new_column_dict, old_type, new_type, column):
    ...
    if is_primary == 1: ## 主键表
        ...
        if old_primary == "N" and new_primary == "Y": ## 非主键变为主键
            ...
            elif new_type_2 == "CHAR":
                filed_line = 'CHAR(' +new_type.split(',')[1]+ ')'
...
def deal_column_del(table, newdate, olddate, del_list, is_primary, old_column_dict):
    ...
    if is_primary == 1: ## 主键表
        ...
        for key in del_list:
            ...
            sql = "select trim(field_nm) from DSA.ORGIN_TABLE_DETAIL where trim(src_stm_id) ='{0}' and trim(tab_code) = '{1}' and trim(field_code)='{2}' and CHANGE_DATE='{3}'".format(syscode,tablenm,key,olddate)
            cursor_dw.execute(sql)
            rows = cursor_dw.fetchall()
...
def deal_column_add(table, newdate, add_list, is_primary, new_column_dict):
    ...
    his_tablename = "ODSHIS."+table.replace('.','_')
    syscode,tablenm = table.split('.')
    tablestr = table.replace('.','_') # 0712 修改
    if is_primary == 1: #主键表
        print(table+ ' has primary_key')
        ## 2.判断添加字段中是否含有主键
        primary_list=[]
        ...
        for key in add_list:
            ...
            sql = "select trim(field_nm) from DSA.ORGIN_TABLE_DETAIL where trim(src_stm_id) ='{0}' and trim(tab_code) = '{1}' and trim(field_code)='{2}' and CHANGE_DATE='{3}'".format(syscode,tablenm,key,newdate)
            cursor_dw.execute(sql)
            rows = cursor_dw.fetchall()
...
def deal_columns(need_tables,olddate,newdate):
    for table in need_tables:
        ...
        ## 判断是否为主键表
        syscode, tablenm = table.split('.')
        sql = "select count(1) from DSA.ORGIN_TABLE_DETAIL where trim(src_stm_id) ='{2}' and trim(tab_code) = '{0}' and change_date='{1}' and primary_key_flag <>''".format(tablenm,olddate,syscode)
        # print(sql)
        cursor_dw.execute(sql)
        rows = cursor_dw.fetchall()
        for i in rows:
