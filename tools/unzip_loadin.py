...
def get_existlist():
    os.chdir("../tools")
    with open("exists_file.txt",'r',encoding='utf-8') as f:
...
def update_existlist(filename, is_dir_flag):
    if is_dir_flag == 0:
        os.chdir("../tools")
    else:
        os.chdir("../../tools")
    with open("exists_file.txt",'a',encoding='utf-8') as f:
        f.write(filename.lstrip('../')+'\n')

def transfer_excel_db2(filename, zone):
    
    dirname = os.listdir(filename + "_files")[0]
    print(dirname)
    if os.path.isdir(filename + "_files/"+dirname):
        print("is dir")
        is_dir_flag = 1  ## 多一层目录
        excel_list = os.listdir(filename + "_files/"+dirname)
        print(excel_list)
        os.chdir(filename + "_files/"+dirname)
    else:
...
if __name__=="__main__":
    ...
    try:
        ...
        if not rows:
            ...
            cursor_dw.execute(sql)
            cursor_dw.commit()
            sql = "alter table DSA.ORGIN_TABLE_DETAIL add primary key(CHANGE_DATE,SRC_STM_ID,TAB_CODE,FIELD_CODE)"
            cursor_dw.execute(sql)
            cursor_dw.commit()
        ...
        for filename in file_list:
            if filename not in exist_filelist:
                ...
                try:
                    load_flag = 1
                    is_dir_flag = transfer_excel_db2(filename,zone)  ## 入库
                # break
                    update_existlist(filename, is_dir_flag)
